...
// Modified for in-place array reversal.
struct TestCase {
...
int main() {
    // Updated test cases for in-place array reversal
    ...
    for (int i = 0; i < test_cases.size(); ++i) {
        ...
        float* d_input = nullptr; // Only one buffer needed for in-place
        ...
        // Copy the modified (reversed) data from Device back to Host
        checkCudaError(cudaMemcpy(h_output.data(), d_input, data_size, cudaMemcpyDeviceToHost));
